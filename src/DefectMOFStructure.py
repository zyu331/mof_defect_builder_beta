...
class DefectMOFStructure():
    ...
    def __init__(self, linkers, nodes, superCell, defectConc, numofdefect, linker_type, charge_comp = 0):
    ...
    def __mul__(self, structure, scaling_matrix, structure_type):
        ...
        for i,v in enumerate(c_lat):
            ...
            for site in structure:
                if np.isnan(site.Nbm):
                    new_NbM = np.nan
                else:
                    new_NbM = site.NbM + i*1
                ...
                s = PeriodicSite(
                    ...
                    to_unit_cell=False,
                    skip_checks=True,
                    LCA = site.LCA + i*original_atom_num,
                    NbM = new_NbM
                )
    ...
    def Sub_with_Capping_agent(self,neighbor_list):
                    ...
                    for _index2_ in coord_indexes_linker:
                        if _index1_ != _index2_:
                            ...
                            if dist < 3 and structure.sites[_index1_].species_string == 'O' : 
                                ...
                                pair.append(_pair_)   
                    if len(_pair_) ==1 and (structure.sites[_index1_].species_string == 'N' or structure.sites[_index1_].species_string == 'O'):
                        pair.append(_pair_) 
            if not pair:
                raise "error"
            
            for paircount, _pair_ in enumerate(pair):
                ...
                if len(_pair_) == 1:
                    ...
                    if site.species_string == 'N':
                        if newMolecuar:
                            newMolecuar = substitute_funcGroup( 'N', _structure_, _pair_, oldMolecular = newMolecuar, charge_comp = paircount )
                        else:
                            newMolecuar = substitute_funcGroup( 'N', _structure_, _pair_, oldMolecular = newMolecuar, charge_comp = paircount )
                    if site.species_string == 'O':
                        if newMolecuar:
                            newMolecuar = substitute_funcGroup( 'O', _structure_, _pair_, oldMolecular = newMolecuar, charge_comp = paircount )
                        else:
                            newMolecuar = substitute_funcGroup( 'O', _structure_, _pair_, oldMolecular = newMolecuar, charge_comp = paircount )
        ...
        if not os.path.isdir(str(self.defectConc)):
            os.mkdir(str(self.defectConc))            
        WriteStructure(str(self.defectConc), output, name = 'POSCAR'+self.linker_type)
